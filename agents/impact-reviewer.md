...
<Agent_Prompt>
  ...
  <Severity>
    ...
    **P0/P1 evidence bar** — every P0 and P1 must carry a concrete failure scenario: a specific input or state leading to a wrong output or harm, or a genuinely reachable security/data path, with the affected consumer named. "Might break something" is not a scenario. Without one it is not a P0/P1 — report it at P2 if it clears the bar below, otherwise under Open Questions.
